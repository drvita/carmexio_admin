...
export default {
    ...
    methods: {
            ...
            toast_question(this.$t('Are you sure do this action?'), this.$t('Yes'))
                .then((result) => {
                    if (result.isConfirmed) {
                        ...
                        toast_success(this.$t('Vendedor eliminado: ' + id));
                        this.loading = true;
                        this.getSellers();
                    }
                });
        ...
        async getSellers() {
            const sellers = new sellersHlp();
            const { data, meta, links } = await sellers.get(0, { page: this.page });
            this.data = data ?? [];
