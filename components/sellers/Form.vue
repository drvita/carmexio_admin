...
export default {
    ...
    methods: {
        ...
        handleBtnSave() {
            ...
            if (!form.status) {
                console.error("[Seller] Form inValid");
            ...
            this.loading = true;
            if (this.form.id) {
                seller.update(this.form.id, data).then(({ data }) => {
                    if (data) {
                        console.log("[Seller] Seller updated:", data.id);
                        toast_success(this.$t('Seller updated'));
                        this.$emit("saved", data.id);
                        this.handleClose();
                    }
                    this.loading = false;
                }).catch(err => {
                    console.error("[Seller] error put:", err?.message ?? err);
            ...
            seller.set(data).then(({ data }) => {
                if (data) {
                    console.log("[Seller] Seller saved:", data.id);
                    toast_success(this.$t('Seller saved') + ". \n " + this.$t('The password is') + ": Password.01#");
                    this.$emit("saved", data.id);
                    this.handleClose();
                }
                this.loading = false;
            }).catch(err => {
                console.error("[Seller] error post:", err?.message ?? err);
                toast_error(this.$t('Sorry, we have error in server, try again later'));
                this.loading = false;
            });
        },
    ...
    computed: {
        ...
        title() {
            return this.form.id ? this.$t('Edit seller') : this.$t('New seller');
        },
        validate() {
